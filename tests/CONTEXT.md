...
| `test_merge.py` | xlmtec merge: happy path, missing dir/config, dtype validation |
| `test_evaluate.py` | xlmtec evaluate: output display, unknown metric, num-samples flag |
| `test_benchmark.py` | xlmtec benchmark: summary output, run_comparison called |
| `test_upload.py` | xlmtec upload: token, private, HF_TOKEN env, merge-adapter flow |
...
- **patch() target = where the name is used, not where it's exported**
  e.g. `xlmtec.data.pipeline.DataPipeline` not `xlmtec.data.DataPipeline`
